...
import org.apache.fineract.portfolio.group.domain.CreateSignatoryDTO;
import org.apache.fineract.portfolio.group.domain.Group;
import org.apache.fineract.portfolio.group.domain.GroupRepository;
import org.apache.fineract.portfolio.group.serialization.CustomApiResponse;
...
public class GroupsApiResource {
    ...
    public String retrieveOne(@Context final UriInfo uriInfo, @PathParam("groupId") @Parameter(description = "groupId") final Long groupId,
        ...
        Collection<ClientData> groupSignatories = null;
        ...
        if (!associationParameters.isEmpty()) {
            if (associationParameters.contains("all")) {
                associationParameters.addAll(
                        Arrays.asList("clientMembers","groupSignatories", "activeClientMembers", "groupRoles", "calendars", "collectionMeetingCalendar"));
            }
            ...
            if (associationParameters.contains("groupSignatories")) {
                groupSignatories = clientReadPlatformService.retrieveGroupSignatories(groupId);
                if (CollectionUtils.isEmpty(groupSignatories)) {
                    groupSignatories = null;
                }
            }
            ...
            group = GroupGeneralData.withAssocations(group, membersOfGroup, groupSignatories, activeClientMembers, groupRoles, calendars,
...
@POST
@Path("add-signatory")
public Response addSignatoryToGroup(@RequestBody @Valid CreateSignatoryDTO signatory) {
    try {
        Group group = groupRepository.getById(signatory.getGroupId());
        Client client = clientRepository.getById(signatory.getClientId());

        // Check if the signatory/client pair already exists in the group
        if (group.getGroupSignatories().contains(client)) {
            return Response.status(Response.Status.BAD_REQUEST)
                .entity(new CustomApiResponse("error", "Signatory/Client already exists in the group.", 400))
                .build();
        }

        // Check if the signatory/client pair already exists in the database
        if (signatoryExists(signatory)) {
            return Response.status(Response.Status.BAD_REQUEST)
                .entity(new CustomApiResponse("error", "Signatory/Client pair already exists.", 400))
                .build();
        }

        // If the signatory/client pair doesn't exist, proceed with adding to the group
        group.addSignatories(client);
        groupRepository.save(group);

        return Response.status(Response.Status.OK)
            .entity(new CustomApiResponse("success", "Signatory/Client added to the group successfully.", 200))
            .build();
    } catch (EntityNotFoundException ex) {
        // Handle the case where the entity is not found
        return Response.status(Response.Status.NOT_FOUND)
            .entity(new CustomApiResponse("error", "Group or Client not found.", 404))
            .build();
    }
}

    private boolean signatoryExists(CreateSignatoryDTO signatory) {
        int count = groupRepository.countByGroupIdAndClientId(signatory.getGroupId(), signatory.getClientId());
        return count > 0;
    }
